...
impl ImageStore {
    ...
    pub async fn get_images(&self, limit: usize) -> Vec<Image> {
        let db_images = &self.database.prepare("SELECT i.*, c.display_name AS category_display_name
            FROM images i
...
WHERE i.id = ?1")
    ...
    pub async fn add_image(&mut self, image: Image) -> Result<Image, ()> {
        let insert_result = &self.database.prepare("INSERT INTO images
